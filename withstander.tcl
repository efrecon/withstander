...
proc PrettyStats { dta args } {
    set str ""
    foreach {name key} $args {
        if { [dict exists $dta $key] } {
            append str "${name}:[dict get $dta $key], "
        }
    }
...
proc collect { id { dta {}} } {
  ...
  if { [info exists ::C_$id] && [llength $dta] } {
    ...
    docker log DEBUG "Computed stats:: [PrettyStats $dta "mem%" memPercent mem mem memLimit memLimit cpu% cpuPercent rx rx tx tx pids pidsCurrent]"
